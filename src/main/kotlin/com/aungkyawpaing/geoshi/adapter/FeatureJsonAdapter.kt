...
internal class FeatureJsonAdapter(
    ...
    companion object {
        ...
        private val KEYS_OPTIONS =
            JsonReader.Options.of(KEY_ID, KEY_TYPE, KEY_GEOMETRY, KEY_PROPERTIES)
    }
    ...
    override fun fromJson(reader: JsonReader): Feature? {
                ...
                1 -> {
                    try {
                        ...
                        throw JsonDataException(
                            ("'type' is not of Feature at ${reader.path}"),
                            exception
                        )
                    }
                    ...
                    if (geometryTypeString != null) {
                        ...
                        when (geometryType) {
                            ...
                            GeometryType.MULTI_LINE_STRING -> {
                                val multiLineString =
                                    multiLineStringJsonAdapter.fromJsonValue(jsonValueMap)
                                if (multiLineString != null) geometry = multiLineString
                            }
                            GeometryType.MULTI_POLYGON -> {
                                val multiPolygon =
                                    multiPolygonJsonAdapter.fromJsonValue(jsonValueMap)
                                if (multiPolygon != null) geometry = multiPolygon
                            }
                            GeometryType.GEOMETRY_COLLECTION -> {
                                val geometryCollection =
                                    geometryCollectionJsonAdapter.fromJsonValue(jsonValueMap)
        ...
        return when (val validation = feature.validate()) {
            is Validation.Valid -> feature
